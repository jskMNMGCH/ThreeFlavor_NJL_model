...
int main() {
    ...
    double T_start = 1.0, T_end = 2.5e2;  // Temperature (MeV)
    double T_step = (T_end - T_start)/(NUM_SETS - 1.0);  // Step size for T (MeV)
    
    double mu_start = 1.0, mu_end=6e2;  // Chemical potential (MeV)
    double mu_step = (mu_end - mu_start)/(NUM_SETS - 1.0); // Step size for mu (MeV)
