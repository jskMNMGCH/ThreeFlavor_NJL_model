...
//  --------------------
    ...
    if ((mu[0] + mu[1] + mu[2]) / 3.0 < 360.0) {
        ...
        memcpy(M_u_guess, M_u_guess_init, sizeof(M_u_guess));
        memcpy(M_d_guess, M_d_guess_init, sizeof(M_d_guess));
        memcpy(M_s_guess, M_s_guess_init, sizeof(M_s_guess));
    } else {
        double M_u_guess_init[2] = {52.5, m[0]};
        double M_d_guess_init[2] = {52.5, m[1]};
        double M_s_guess_init[2] = {464.4, m[2]};
        memcpy(M_u_guess, M_u_guess_init, sizeof(M_u_guess));
        memcpy(M_d_guess, M_d_guess_init, sizeof(M_d_guess));
        memcpy(M_s_guess, M_s_guess_init, sizeof(M_s_guess));
    }
